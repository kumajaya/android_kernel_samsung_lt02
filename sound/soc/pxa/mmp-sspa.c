...
/*
 * logically system can enter suspend only when there is no active
 * audio stream. here disable sysclk to ensure audio sspa clock are
 * disabled. also would reserve audio pll register and disable audio
 * clock in audio machine suspend function.
 */
#ifdef CONFIG_PM_SLEEP
static int mmp_sspa_suspend(struct device *dev)
{
...
/*
 * here we resume the sspa sysclk no matter there is active audio
 * stream or not. considering that android would always hold an
 * active audio playback stream.
 */
static int mmp_sspa_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct sspa_priv *priv = platform_get_drvdata(pdev);
...
static SIMPLE_DEV_PM_OPS(mmp_sspa_pm_ops, mmp_sspa_suspend, mmp_sspa_resume);
