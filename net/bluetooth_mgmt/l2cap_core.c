...
static struct l2cap_conn *prev_rfc_conn[2];
#endif
...
static void __l2cap_chan_add(struct l2cap_conn *conn, struct l2cap_chan *chan)
{
	...
	case 0x03:
		rfc_conn = conn;
#ifdef CONFIG_BT_W8787
		if (prev_rfc_conn[0] == NULL)
		{
			prev_rfc_conn[0] = conn;
		}
		else if(prev_rfc_conn[1] == NULL)
		{
			prev_rfc_conn[1] = conn;
		}
#endif
	...
	if (prev_rfc_conn[0] != NULL)
	{
		if ( prev_rfc_conn[0] != conn)
		{
			if (!(prev_rfc_conn[0]->hcon->link_mode & HCI_LM_MASTER))
			{
				if (prev_rfc_conn[0]->hcon->mode == HCI_CM_SNIFF)
				{
					hci_conn_enter_active_mode(prev_rfc_conn[0]->hcon, BT_POWER_FORCE_ACTIVE_ON);
				}
				hci_conn_switch_role(prev_rfc_conn[0]->hcon, 0x00);
			}
		}
		else
		{
			...
			{
				hci_conn_switch_role(prev_rfc_conn[0]->hcon, 0x00);
				prev_rfc_conn[0] = NULL;
				rfc_conn = NULL;
				if (prev_rfc_conn[1] != NULL)
				{
					prev_rfc_conn[0] = prev_rfc_conn[1];
	...
	else
	{
		if (av_conn != NULL &&  (hid_conn != NULL || rfc_conn != NULL))
		{
			if (hid_conn == conn)
			{
				if (av_conn->hcon->mode == HCI_CM_SNIFF)
					hci_conn_enter_active_mode(av_conn, BT_POWER_FORCE_ACTIVE_ON);
			}
