...
void hci_setup_sync(struct hci_conn *conn, __u16 handle)
{
	...
	struct hci_cp_enhanced_setup_sync_conn wb_cp;
	BT_DBG("%p", conn);
	...
	if (conn->hdev->is_wbs) {
		BT_DBG("setup hci connection with wide band sco mode");

		wb_cp.handle   = cpu_to_le16(handle);
		wb_cp.tx_bandwidth   = cpu_to_le32(0x00001f40);
		wb_cp.rx_bandwidth   = cpu_to_le32(0x00001f40);
		wb_cp.tx_coding_fmt.number = BTM_CODING_MSBC;
		wb_cp.rx_coding_fmt.number = BTM_CODING_MSBC;
		wb_cp.tx_codec_frame_size = cpu_to_le16(0x003c);
		wb_cp.rx_codec_frame_size = cpu_to_le16(0x003c);

		wb_cp.in_bandwidth = cpu_to_le32(0x00003e80);
		wb_cp.out_bandwidth = cpu_to_le32(0x00003e80);
		wb_cp.in_coding_fmt.number = BTM_CODING_ULAW;
		wb_cp.out_coding_fmt.number = BTM_CODING_ULAW;

		wb_cp.in_coded_data_size = cpu_to_le16(0x0008);
		wb_cp.out_coded_data_size = cpu_to_le16(0x0008);
		wb_cp.in_pcm_data_fmt = BTM_PCM_DATA_ONES;
		wb_cp.out_pcm_data_fmt = BTM_PCM_DATA_ONES;
		wb_cp.in_pcm_smp_msb_pos = 0x00;
		wb_cp.out_pcm_smp_msb_pos = 0x00;
		wb_cp.in_data_path = BTM_DATA_PATH_PCM;
		wb_cp.out_data_path = BTM_DATA_PATH_PCM;
		wb_cp.in_trans_unit_size = 0x00;
		wb_cp.out_trans_unit_size = 0x00;
		wb_cp.max_latency    = cpu_to_le16(0xffff);
		wb_cp.pkt_type = cpu_to_le16(conn->pkt_type);
		wb_cp.retrans_effort = 0xff;

		hci_send_cmd(hdev, HCI_OP_ENHANCED_SETUP_SYNC_CONN,
				sizeof(wb_cp), &wb_cp);
	}
	else {
		BT_DBG("setup hci connection with narrow band sco mode");
		cp.handle   = cpu_to_le16(handle);
		cp.pkt_type = cpu_to_le16(conn->pkt_type);
