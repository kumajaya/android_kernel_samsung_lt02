...
extern int spa_fuelgauge_current;
...
#else
	STC31xx_WriteWord(STC311x_REG_SOC,GG_Ram.reg.HRSOC*512);  /*   restore SOC */
#endif
#else  
  /* rewrite ocv to start SOC with updated OCV curve */
  STC31xx_WriteWord(STC311x_REG_OCV,ocv);
...
#else
	if(BattData.BattState > BATT_IDLE && BattData.SOC >= 990 && BattData.SOC < 995 && BattData.AvgCurrent > 190)
#endif
      {
        BattData.SOC = 990;
...
static int read_voltage(int *vbat)
{
	...
	*vbat =  chip->batt_voltage;
	spa_fuelgauge_current = chip->batt_current;
...
static int reset_fuelgauge(void)
{
	//STC311x_Reset(); To not be used directly
  GasGauge_Reset();
...
printk("stc311x_probe\n");
 ...
 printk("stc311x probe debug 5\n");	
	...
	ret = spa_bat_register_fuelgauge_reset(reset_fuelgauge);
	if(ret)
	{
		printk("%s fail to register reset_fuelgauge function\n", __func__);
	}
#else
