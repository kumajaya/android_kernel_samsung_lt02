...
static
struct _rx_buffer_st *smsspi_handle_unused_bytes_buf(struct _spi_dev *dev,
						     struct _rx_buffer_st *buf,
						     int offset,
	...
	for (i = 0; i < len; i++, ptr++) {
		...
		case RxsData:
			...
			if (dev->cb.msg_found_cb) {
				align_fix = 0;
				if (dev->rxPacket.msg_flags &
				    MSG_HDR_FLAG_SPLIT_MSG_HDR) {
					align_fix =
					    (dev->
					     rxPacket.msg_flags >> 8) & 0x3;
