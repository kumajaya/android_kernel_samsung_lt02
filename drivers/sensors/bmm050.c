...
char bmm050_init(struct bmm050 *bmm050)
{
	...
	comres = p_bmm050->bus_read(p_bmm050->dev_addr,
			BMM050_CHIP_ID, a_data_u8r, 1);
	p_bmm050->company_id = a_data_u8r[0];
	pr_info("%s: chip id os 0x%02x\n", __func__, p_bmm050->company_id);
...
char bmm050_set_functional_state(
...
char bmm050_get_functional_state(
		unsigned char *functional_state)
{
	char comres = 0;
...
char bmm050_read_mdataXYZ(struct bmm050_mdata *mdata)
{
	...
	struct {
		signed short raw_dataX;
		signed short raw_dataY;
		signed short raw_dataZ;
	...
	if (p_bmm050 == BMM050_NULL) {
		...
		raw_dataXYZ.raw_dataX = (signed short)((((signed short)
		...
		raw_dataXYZ.raw_dataY = (signed short)((((signed short)
		...
		raw_dataXYZ.raw_dataZ = (signed short)((((signed short)
...
#ifdef ENABLE_FLOAT
char bmm050_read_mdataXYZ_float(
		...
		unsigned char *data, unsigned char len)
{
	...
	if (p_bmm050 == BMM050_NULL) {
		...
		comres = p_bmm050->bus_write(p_bmm050->dev_addr,
			addr, data, len);
	}
	return comres;
}
...
{
	char comres = 0;
	unsigned char data[6], v_result_u8r = 0x00;
	if (p_bmm050 == BMM050_NULL) {
		comres = E_BMM050_NULL_PTR;
	} else {
		comres = p_bmm050->bus_read(
		...
		if (data[1] & 0x80)
			*x = (int)((data[0]>>3)|(((0x7f)&(data[1]))<<5)) * -1;
		else
			*x = (int)((data[0]>>3)|(data[1]<<5));

		if (data[3] & 0x80)
			*y = (int)((data[2]>>3)|(((0x7f)&(data[3]))<<5)) * -1;
		else
			*y = (int)((data[2]>>3)|(data[3]<<5));

		if (data[5] & 0x80)
			*z = (int)((data[4]>>1)|(((0x7f)&(data[5]))<<7)) * -1;
		else
			*z = (int)((data[4]>>1)|(data[5]<<7));
		...
		v_result_u8r = BMM050_GET_BITSLICE(data[4],
				BMM050_DATAZ_LSB_TESTZ);

		v_result_u8r = (v_result_u8r << 1);
		v_result_u8r = (v_result_u8r | BMM050_GET_BITSLICE(
					data[2], BMM050_DATAY_LSB_TESTY));

		v_result_u8r = (v_result_u8r << 1);
		v_result_u8r = (v_result_u8r | BMM050_GET_BITSLICE(
					data[0], BMM050_DATAX_LSB_TESTX));

		*self_testxyz = v_result_u8r;
	}

	pr_info("%s : comres = 0x%02x, v_result_u8r = 0x%02x\n", __func__,
			comres, v_result_u8r);
	return comres;
}
...
char bmm050_perform_advanced_selftest(
		signed short *diff_z)
{
	char comres;
	signed short result_positive, result_negative;
...
char bmm050_init_trim_registers(void)
{
	...
	p_bmm050->dig_z2 = (signed short)((((signed short)(
						(signed char)a_data_u8r[1])) <<
				SHIFT_LEFT_8_POSITION) | a_data_u8r[0]);

	comres |= p_bmm050->bus_read(p_bmm050->dev_addr,
			BMM050_DIG_Z3_LSB, a_data_u8r, 2);
	p_bmm050->dig_z3 = (signed short)((((signed short)(
						(signed char)a_data_u8r[1])) <<
				SHIFT_LEFT_8_POSITION) | a_data_u8r[0]);

	comres |= p_bmm050->bus_read(p_bmm050->dev_addr,
			BMM050_DIG_Z4_LSB, a_data_u8r, 2);
	p_bmm050->dig_z4 = (signed short)((((signed short)(
						(signed char)a_data_u8r[1])) <<
				SHIFT_LEFT_8_POSITION) | a_data_u8r[0]);
...
char bmm050_get_repetitions_XY(
...
char bmm050_set_repetitions_XY(
		unsigned char no_repetitions_xy)
{
	char comres = 0;
	unsigned char v_data_u8r;
...
signed short bmm050_compensate_X(signed short mdata_x, unsigned short data_R)
{
	signed short inter_retval;
	if (mdata_x != BMM050_FLIP_OVERFLOW_ADCVAL  /* no overflow */
	   ) {
		inter_retval = ((signed short)(((unsigned short)
				((((signed int)p_bmm050->dig_xyz1) << 14) /
				 (data_R != 0 ? data_R : p_bmm050->dig_xyz1))) -
				((unsigned short)0x4000)));
		inter_retval = ((signed short)((((signed int)mdata_x) *
				((((((((signed int)p_bmm050->dig_xy2) *
			      ((((signed int)inter_retval) *
				((signed int)inter_retval)) >> 7)) +
			     (((signed int)inter_retval) *
			      ((signed int)(((signed short)p_bmm050->dig_xy1)
			      << 7)))) >> 9) +
			   ((signed int)0x100000)) *
			  ((signed int)(((signed short)p_bmm050->dig_x2) +
			  ((signed short)0xA0)))) >> 12)) >> 13)) +
			(((signed short)p_bmm050->dig_x1) << 3);
	} else {
		/* overflow */
...
signed int bmm050_compensate_X_s32 (signed short mdata_x, unsigned short data_R)
{
	signed int retval;

	retval = bmm050_compensate_X(mdata_x, data_R);
	if (retval == (signed int)BMM050_OVERFLOW_OUTPUT)
		retval = BMM050_OVERFLOW_OUTPUT_S32;
	return retval;
}

#ifdef ENABLE_FLOAT
float bmm050_compensate_X_float (signed short mdata_x, unsigned short data_R)
{
	float inter_retval;
	if (mdata_x != BMM050_FLIP_OVERFLOW_ADCVAL	/* no overflow */
	   ) {
...
signed short bmm050_compensate_Y(signed short mdata_y, unsigned short data_R)
{
	signed short inter_retval;
	if (mdata_y != BMM050_FLIP_OVERFLOW_ADCVAL  /* no overflow */
		...
		inter_retval = ((signed short)(((unsigned short)(((
			...
			((unsigned short)0x4000)));
		inter_retval = ((signed short)((((signed int)mdata_y) *
				((((((((signed int)
				       p_bmm050->dig_xy2) *
				      ((((signed int) inter_retval) *
					((signed int)inter_retval)) >> 7)) +
				     (((signed int)inter_retval) *
				      ((signed int)(((signed short)
				      p_bmm050->dig_xy1) << 7)))) >> 9) +
				   ((signed int)0x100000)) *
				  ((signed int)(((signed short)p_bmm050->dig_y2)
					  + ((signed short)0xA0))))
				 >> 12)) >> 13)) +
			(((signed short)p_bmm050->dig_y1) << 3);
	} else {
		/* overflow */
		inter_retval = BMM050_OVERFLOW_OUTPUT;
	}
...
signed int bmm050_compensate_Y_s32 (signed short mdata_y, unsigned short data_R)
{
	signed int retval;
...
#ifdef ENABLE_FLOAT
float bmm050_compensate_Y_float(signed short mdata_y, unsigned short data_R)
{
	float inter_retval;
	if (mdata_y != BMM050_FLIP_OVERFLOW_ADCVAL /* no overflow */
	   ) {
...
signed short bmm050_compensate_Z(signed short mdata_z, unsigned short data_R)
{
	signed int retval;
	if ((mdata_z != BMM050_HALL_OVERFLOW_ADCVAL)	/* no overflow */
	   ) {
		retval = (((((signed int)(mdata_z - p_bmm050->dig_z4)) << 15) -
					((((signed int)p_bmm050->dig_z3) *
					  ((signed int)(((signed short)data_R) -
						  ((signed short)
						   p_bmm050->dig_xyz1))))>>2)) /
				(p_bmm050->dig_z2 +
				 ((signed short)(((((signed int)
					 p_bmm050->dig_z1) *
					 ((((signed short)data_R) << 1)))+
						 (1<<15))>>16))));
...
signed int bmm050_compensate_Z_s32(signed short mdata_z, unsigned short data_R)
{
	signed int retval;
	if (mdata_z != BMM050_HALL_OVERFLOW_ADCVAL) {
		retval = (((((signed int)(mdata_z - p_bmm050->dig_z4)) << 15) -
			((((signed int)p_bmm050->dig_z3) *
			((signed int)(((signed short)data_R) -
			((signed short)p_bmm050->dig_xyz1))))>>2)) /
			(p_bmm050->dig_z2 +
			((signed short)(((((signed int)p_bmm050->dig_z1) *
			((((signed short)data_R) << 1)))+(1<<15))>>16))));
	} else {
		retval = BMM050_OVERFLOW_OUTPUT_S32;
	}
	return retval;
...
#ifdef ENABLE_FLOAT
float bmm050_compensate_Z_float (signed short mdata_z, unsigned short data_R)
{
	float inter_retval;
	/* no overflow */
	if (mdata_z != BMM050_HALL_OVERFLOW_ADCVAL) {
...
char bmm050_get_raw_xyz(struct bmm050_mdata *mdata)
{
	...
	if (p_bmm050 == BMM050_NULL) {
		...
		p_bmm050->bus_read(p_bmm050->dev_addr,
				BMM050_R_LSB, &data_ready, 1);

		if ((data_ready & 0x01) == 0) {
			pr_info("%s: data_ready  = 0\n", __func__);
			mdata->datax = 0;
			mdata->datay = 0;
			mdata->dataz = 0;
			return 0;
		...
		a_data_u8r[0] = BMM050_GET_BITSLICE(a_data_u8r[0],
				BMM050_DATAX_LSB_VALUEX);
		mdata->datax = (signed short)((((signed short)
						((signed char)a_data_u8r[1]))
		...
		a_data_u8r[2] = BMM050_GET_BITSLICE(a_data_u8r[2],
				BMM050_DATAY_LSB_VALUEY);
		mdata->datay = (signed short)((((signed short)
						((signed char)a_data_u8r[3]))
					<< SHIFT_LEFT_5_POSITION)
				| a_data_u8r[2]);
		...
		a_data_u8r[4] = BMM050_GET_BITSLICE(a_data_u8r[4],
				BMM050_DATAZ_LSB_VALUEZ);
		mdata->dataz = (signed short)((((signed short)
						((signed char)a_data_u8r[5]))
