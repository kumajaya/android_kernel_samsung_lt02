		...
		struct mldl_cfg *mldl_cfg, void *mlsl_handle)
{
	...
	if (sw_rev == 0) {
		...
		if (key == 0) {
			...
			MPL_LOGE("indicates device is either ");
			MPL_LOGE("incompatible or an MPU3050\n");
		...
		if (prod_rev_map[index].silicon_rev != MPU_SILICON_REV_B1) {
			MPL_LOGE("MPL device is not MPU6050B1 (%d)\n", key);
			return INV_ERROR_INVALID_MODULE;
		}
	...
	/* if valid, use the software product key */
		...
		MPL_LOGE("Software revision key is out of range [0..%d]\n",
				 ...
				 int slave_id)
{
	...
	if (slave_len > BITS_SLV_LENG) {
		MPL_LOGW("Slave burst read allowed maximum is req. %d)\n",
				 slave_len);
		   ...
		   unsigned long sensors)
{
	...
	void *slave_handle[EXT_SLAVE_NUM_TYPES];
	/* enable mpu GPIO irq when resume*/
		    ...
		    unsigned long sensors)
{
	...
	/*disable mpu GPIO irq when enter suspend*/
	inv_serial_single_write(gyro_handle, mldl_cfg->mpu_chip_info->addr,
				MPUREG_INT_ENABLE, 0);
