	...
	unsigned char const *data)
{
	...
	if ((mem_addr & 0xFF) + length > MPU_MEM_BANK_SIZE) {
		pr_err("memory read length (%d B) extends ", length);
		pr_err("beyond its limits (%d) if started at location %d\n",
	...
	unsigned char *data)
{
	...
	if ((mem_addr & 0xFF) + length > MPU_MEM_BANK_SIZE) {
		printk
		    (KERN_INFO "memory read length (%d B)\n", length);
		printk
		    (KERN_INFO "beyond its limits (%d) if started at %d\n",
		     MPU_MEM_BANK_SIZE, mem_addr & 0xFF);
		return INV_ERROR_INVALID_PARAMETER;
	}
	while (bytes_read < length) {
