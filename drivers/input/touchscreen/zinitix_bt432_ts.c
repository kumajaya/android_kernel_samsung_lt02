...
#include <linux/input/zinitix_bt432_ts.h>
...
/* + to support AT */
static ssize_t fw_version_show_phone(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t fw_version_show_panel(struct device *dev,
	struct device_attribute *attr, char *buf);

static DEVICE_ATTR(tsp_firm_version_phone, S_IRUGO | S_IWUSR | S_IWGRP,
	          fw_version_show_phone, NULL);	/* PHONE */
static DEVICE_ATTR(tsp_firm_version_panel, S_IRUGO | S_IWUSR | S_IWGRP,
		   fw_version_show_panel, NULL);/*PART*/
/* - to support AT */
...
static struct regulator *tsp_regulator_3_3;
static struct regulator *tsp_regulator_1_8;
...
static bool ts_power_control(struct zinitix_touch_dev *touch_dev, u8 ctl)
{
	...
	if (tsp_regulator_3_3 == NULL) {
		tsp_regulator_3_3 = regulator_get(&touch_dev->client->dev,
			"v_tsp_3v");

		if (IS_ERR(tsp_regulator_3_3)) {
			ret = PTR_ERR(tsp_regulator_3_3);
			zinitix_printk("can not get TSP AVDD 3V, ret=%d\n",
				ret);
		...
		ret = regulator_set_voltage(tsp_regulator_3_3,
			3000000, 3000000);
		if (ret) {
			zinitix_printk("can not set voltage TSP AVDD 3V, ret=%d\n",
				ret);
		...
		zinitix_printk("3v regulator_enable ret %d\n", ret);
		if (ret) {
			zinitix_printk("can not enable TSP AVDD 3V, ret=%d\n",
				ret);
			return ret;
		}
	}
	...
	if (tsp_regulator_1_8 == NULL) {
		tsp_regulator_1_8 = regulator_get(&touch_dev->client->dev,
			"v_tsp_1v8");

		if (IS_ERR(tsp_regulator_1_8)) {
			ret = PTR_ERR(tsp_regulator_1_8);
			zinitix_printk("can not get TSP AVDD 1.8V, ret=%d\n",
				ret);
			tsp_regulator_1_8 = NULL;
			return ret;
		}

		ret = regulator_set_voltage(tsp_regulator_1_8,
			1800000, 1800000);
		if (ret) {
			zinitix_printk("can not set voltage TSP AVDD 1.8V, ret=%d\n",
				ret);
		...
		zinitix_printk("1.8v regulator_enable ret %d\n", ret);
		if (ret) {
			zinitix_printk("can not enable TSP AVDD 1.8V, ret=%d\n",
				ret);
			return ret;
		}
	}
	...
	if (ctl == TS_POWER_OFF) {
		...
		if (regulator_is_enabled(tsp_regulator_3_3)) {
			...
			zinitix_printk("3v regulator_disable ret = %d\n", ret);
			if (ret) {
				zinitix_printk("can not disable TSP AVDD 3V, ret=%d\n",
					ret);
				return ret;
			}
		}
		if (regulator_is_enabled(tsp_regulator_1_8)) {
			...
			zinitix_printk("1.8v regulator_disable ret %d\n", ret);
			if (ret) {
				zinitix_printk("can not disable TSP AVDD 1.8V, ret=%d\n",
					ret);
				return ret;
			}
		}
	...
	} else if (ctl == TS_POWER_ON_SEQUENCE) {
...
static bool ts_mini_init_touch(struct zinitix_touch_dev *touch_dev)
{
	...
	zinitix_debug_msg("firmware check checksum\r\n");
	...
	if (chip_check_sum != 0x55aa) {
		zinitix_printk("checksum error(0x%04x)\r\n", chip_check_sum);
		goto fail_mini_init;
	}
#endif
	...
	zinitix_printk("touch max x = %d\r\n",
		touch_dev->cap_info.x_resolution);
	zinitix_printk("touch max y = %d\r\n",
	...
	zinitix_printk("successfully mini initialized\r\n");

	return true;

fail_mini_init:
	zinitix_printk("early mini init fail\n");
	ts_power_control(touch_dev, TS_POWER_OFF);
	ts_power_control(touch_dev, TS_POWER_ON_SEQUENCE);

	if (ts_init_touch(touch_dev) == false) {
		zinitix_printk("fail initialized\r\n");
		return false;
	}
...
retry_upgrade:
	ts_power_control(touch_dev, TS_POWER_OFF);
	ts_power_control(touch_dev, TS_POWER_ON);
	mdelay(10);

	if (ts_write_reg(touch_dev->client,
		0xc000, 0x0001) != I2C_SUCCESS) {
		zinitix_printk("power sequence error (vendor cmd enable)\n");
		goto fail_upgrade;
	}
	udelay(10);
	if (ts_read_data(touch_dev->client,
		0xcc00, (u8 *)&chip_code, 2) < 0) {
		zinitix_printk("fail to read chip code\n");
		goto fail_upgrade;
	}
	zinitix_printk("chip code = 0x%x\n", chip_code);
	...
	if (ts_write_reg(touch_dev->client,
		0xc003, 0x0001) != I2C_SUCCESS){
		zinitix_printk("fail to write nvm vpp on\n");
		goto fail_upgrade;
	}

	if (ts_write_reg(touch_dev->client,
		0xc104, 0x0001) != I2C_SUCCESS){
	...
	if (ts_write_reg(touch_dev->client,
		0xc003, 0x0000) != I2C_SUCCESS){
		zinitix_printk("nvm write vpp off\n");
		goto fail_upgrade;
	}

	if (ts_write_reg(touch_dev->client,
		0xc104, 0x0000) != I2C_SUCCESS){
...
fail_upgrade:
	...
	if (retry_cnt++ < ZINITIX_INIT_RETRY_CNT) {
		zinitix_printk("upgrade fail, cnt %d\n", retry_cnt);
		goto retry_upgrade;
	}
...
retry_init:
	...
	zinitix_printk("touch reg data version = %d\r\n",
		reg_data_version);

	if ((touch_dev->factory_update != 1) &&
		(ts_check_need_upgrade(touch_dev,
		firmware_version, minor_firmware_version,
		reg_data_version, touch_dev->cap_info.hw_id) == true)) {
		zinitix_printk("start oneshot upgrade firmware\n");

		if (ts_upgrade_firmware(touch_dev, m_firmware_data,
			touch_dev->cap_info.ic_fw_size) == false) {
			zinitix_printk("oneshot upgrade fail\n");
			goto fail_init;
		}

		if (ts_hw_calibration(touch_dev) == false) {
			zinitix_printk("hw calibration fail\n");
			goto fail_init;
		}
	...
	/* initialize */
	touch_dev->cap_info.x_resolution = touch_dev->pdata->x_max;
	touch_dev->cap_info.y_resolution = touch_dev->pdata->y_max;
	if (ts_write_reg(touch_dev->client, ZINITIX_X_RESOLUTION,
		(u16)(touch_dev->cap_info.x_resolution)) != I2C_SUCCESS)
		goto fail_init;
	...
	zinitix_printk("max supported finger num = %d\r\n",
		touch_dev->cap_info.multi_fingers);
	touch_dev->cap_info.gesture_support = 0;
	zinitix_printk("set other configuration\r\n");
	...
	zinitix_printk("esd timer register = %d\r\n", reg_val);
	zinitix_printk("successfully initialized\r\n");

	return true;

fail_init:
	zinitix_printk("init fail\n");
	touch_dev->factory_update = 0;
	...
	if (++retry_cnt <= ZINITIX_INIT_RETRY_CNT) {
		...
		zinitix_printk("retry to initiallize, cnt = %d\r\n", retry_cnt);

#if TOUCH_FORCE_UPGRADE
		zinitix_printk("start force upgrade firmware\n");

		if (ts_upgrade_firmware(touch_dev, m_firmware_data,
			touch_dev->cap_info.ic_fw_size) == false) {
			zinitix_printk("force upgrade fail\n");
			return false;
		}
		mdelay(100);
...
static void zinitix_parsing_data(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev == NULL) {
		zinitix_debug_msg("touch_dev == NULL?\n");
		return;
	}
	...
	if (touch_dev->work_proceedure != TS_NO_WORK) {
		zinitix_debug_msg("other process occupied..\r\n");
		udelay(DELAY_FOR_SIGNAL_DELAY);
		if (!gpio_get_value(touch_dev->int_gpio_num)) {
			ts_write_cmd(touch_dev->client,
				ZINITIX_CLEAR_INT_STATUS_CMD);
	...
	/* maybe desirable reset */
	if (ts_read_coord(touch_dev) == false ||
		touch_dev->touch_info.status == 0xffff
		|| touch_dev->touch_info.status == 0x1) {
		zinitix_printk("couldn't read touch_dev coord. maybe chip is dead\r\n");
		ts_power_control(touch_dev, TS_POWER_OFF);
		ts_power_control(touch_dev, TS_POWER_ON_SEQUENCE);

		zinitix_printk("clear all reported points\r\n");
		zinitix_clear_report_data(touch_dev);
	...
	if (reported == true || !zinitix_bit_test(
		touch_dev->touch_info.status, BIT_PT_EXIST)) {
		for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
			prev_sub_status =
...
/*		if (zinitix_bit_test(sub_status, SUB_BIT_DOWN)
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			if (!m_ts_debug_mode &&	!zinitix_bit_test(
				prev_sub_status, SUB_BIT_EXIST) &&
...
#if defined(ZCONFIG_PM_SLEEP) && !defined(CONFIG_HAS_EARLYSUSPEND)
const struct dev_pm_ops zinitix_dev_pm_ops = {
	#if defined(CONFIG_PM_RUNTIME)
	SET_RUNTIME_PM_OPS(zinitix_suspend, zinitix_resume, NULL)
	#else
	SET_SYSTEM_SLEEP_PM_OPS(zinitix_suspend, zinitix_resume)
	#endif
};
#endif
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (value == TOUCH_DND_MODE) {
		if (ts_write_reg(misc_touch_dev->client, ZINITIX_DND_N_COUNT,
			...
			zinitix_printk("Fail to set ZINITIX_DND_N_COUNT %d.\r\n",
				SEC_DND_N_COUNT);
		if (ts_write_reg(misc_touch_dev->client, ZINITIX_AFE_FREQUENCY,
			SEC_DND_FREQUENCY) != I2C_SUCCESS)
			zinitix_printk("Fail to set ZINITIX_AFE_FREQUENCY %d.\r\n",
				SEC_DND_FREQUENCY);
	...
	if (ts_write_reg(misc_touch_dev->client, ZINITIX_TOUCH_MODE,
		...
		zinitix_printk("Fail to set ZINITX_TOUCH_MODE %d.\r\n"
			, misc_touch_dev->touch_mode);
...
static void get_threshold(void *device_data)
{
	...
	ret = ts_read_data(info->client, ZINITIX_SENSITIVITY,
		(u8 *)&threshold, 2);
...
static void run_intensity_read(void *device_data)
{
	...
	get_raw_data(info, (u8 *)info->ref_data, 10);
...
static void fw_update(void *device_data)
{
	...
	case BUILT_IN:
		zinitix_printk("fw update - BUILT_IN\n");

		info->factory_update = 1;
	...
	case UMS:
		...
		if (IS_ERR(fp)) {
			zinitix_printk("file %s open error:%d\n",
				fw_path, (s32)fp);
			info->cmd_state = 3;
			goto err_open;
		}
		...
		if (fsize != info->cap_info.ic_fw_size) {
			zinitix_printk("invalid fw size!!\n");
			info->cmd_state = 3;
			goto err_open;
		}
		...
		if (!buff) {
			zinitix_printk("fail to alloc buffer for fw\n");
			info->cmd_state = 3;
			goto err_alloc;
		}
		...
		set_fs(old_fs);
		zinitix_printk("ums fw is loaded!!\n");
	...
	default:
		zinitix_printk("invalid fw file type!!\n");
		goto not_support;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, sizeof(buff), "ZI%06X", version);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case TOUCH_IOCTL_SET_DEBUGMSG_STATE:
		...
		if (nval) {
			zinitix_printk("on debug mode (%d)\n",
				nval);
		} else {
			zinitix_printk("off debug mode (%d)\n",
				nval);
		}
	...
	case TOUCH_IOCTL_VARIFY_UPGRADE_SIZE:
		...
		zinitix_printk("firmware size = %d\r\n", sz);
...
fail_hw_cal:
	...
	case TOUCH_IOCTL_GET_REG:
		...
		if (copy_to_user(reg_ioctl.val, (u8 *)&nval, 4)) {
			misc_touch_dev->work_proceedure = TS_NO_WORK;
			up(&misc_touch_dev->work_proceedure_lock);
			zinitix_printk("error : copy_to_user\n");
			return -1;
		}
...
#if SEC_TSP_FACTORY_TEST
static ssize_t zinitix_ts_proc_read(char *buf, char **start, off_t off,
...
static ssize_t zinitix_ts_proc_write(struct file *filp,
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if (strcmp(client->name, ZINITIX_BT432_TS_DEVICE) != 0) {
		zinitix_printk("not zinitix driver.\r\n");
		return -1;
	}
	...
	if (!zinitix_workqueue) {
		zinitix_printk("unabled to create touch thread \r\n");
	...
	ret = gpio_request(pdata->gpio_int, "zinitix_irq_gpio");
	if (ret) {
...
#endif
	/* power on */
	if (ts_power_control(touch_dev, TS_POWER_ON_SEQUENCE) == false) {
	...
	touch_dev->input_dev->name = ZINITIX_BT432_TS_DEVICE;
	...
	zinitix_printk("register %s input device \r\n",
	...
	if (touch_dev->irq < 0)
		zinitix_printk("should define GPIO_TOUCH_IRQ.\r\n");
#endif
	zinitix_printk("request irq (irq = %d, pin = %d) \r\n",
	...
	ret = request_threaded_irq(touch_dev->irq, NULL, zinitix_touch_work,
		...
		ZINITIX_BT432_TS_DEVICE, touch_dev);
#else
	ret = request_irq(touch_dev->irq, ts_int_handler,
		IRQF_TRIGGER_LOW, ZINITIX_BT432_TS_DEVICE, touch_dev);
	/*	IRQF_TRIGGER_FALLING, ZINITIX_BT432_TS_DEVICE, touch_dev); */
#endif
	if (ret) {
	...
	if (ret)
		zinitix_printk("Fail to register touch misc device.\n");
	...
	if (IS_ERR(sec_touchscreen_dev))
		zinitix_printk("Failed to create device for the sysfs(%s)\n"
			, dev_attr_touchkey_menu.attr.name);

	ret = sysfs_create_group(&sec_touchscreen_dev->kobj
		, &sec_touch_attributes_group);
	if (ret)
		zinitix_printk("Failed to create sysfs(%s).\n"
			, dev_attr_touchkey_menu.attr.name);
	...
	if (device_create_file(sec_touchkey, &dev_attr_touchkey_threshold) < 0)
       ...
       sec_touchkey = device_create(sec_class, NULL, 0
        ...
        if (device_create_file(sec_touchkey, &dev_attr_tsp_firm_version_phone) < 0)
		zinitix_printk("Failed to create device file(%s)!\n"
			, dev_attr_tsp_firm_version_phone.attr.name);
	if (device_create_file(sec_touchkey, &dev_attr_tsp_firm_version_panel) < 0)
		zinitix_printk("Failed to create device file(%s)!\n"
			, dev_attr_tsp_firm_version_phone.attr.name);


	proc_file = create_proc_entry("driver/mms_ts", 0644, NULL);
	if (proc_file) {
                proc_file->read_proc = zinitix_ts_proc_read;
                proc_file->write_proc = (write_proc_t  *)zinitix_ts_proc_write;
        } else
                pr_info("TS proc file create failed!\n");
#endif
...
static int zinitix_touch_remove(struct i2c_client *client)
{
	...
	zinitix_printk("zinitix_touch_remove+ \r\n");
	down(&touch_dev->work_proceedure_lock);
#if !USE_THREADED_IRQ
	if (touch_dev->work_proceedure != TS_NO_WORK)
		flush_work(&touch_dev->work);
