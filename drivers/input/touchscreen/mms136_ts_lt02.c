...
static void ts_power_enable(int en)
{
	...
	int tsp_ldo_on = 0;
#if 0
	if (system_rev >= LT02_BRINGUP_01) {
#endif
		tsp_ldo_on = mfp_to_gpio(GPIO020_GPIO_20);
		if (gpio_request(tsp_ldo_on, "TSP_LDO_ON")) {
		...
		if(touch_regulator_3v0 == NULL) {
			touch_regulator_3v0 = regulator_get(NULL,"v_ldo14_3v");
		...
		if(touch_regulator_1v8 == NULL) {
			touch_regulator_1v8 = regulator_get(NULL,"v_ldo8");
...
#endif		/* hy82: remove feature of bringup board */
	if (en == TOUCH_ON) {
		if (!is_power_on) {
			is_power_on = 1;
#if defined(CONFIG_MACH_LT02) || defined(CONFIG_MACH_COCOA7)
			...
			if (system_rev >= LT02_BRINGUP_01)
#endif
				gpio_direction_output(tsp_ldo_on, 1);
#if 0
			else {
				/* On rev0.0 lt02 board */
		...
		if (is_power_on) {
			...
			if (system_rev >= LT02_BRINGUP_01) {
#endif
				gpio_direction_output(tsp_ldo_on, 0);
				gpio_free(tsp_ldo_on);
/* hy82: remove feature of bringup board */
#if 0
			} else {
				/* On rev0.0 lt02 board */
...
static int __attribute__((unused)) mms_set_config(struct i2c_client *client, u8 reg, u8 value)
{
	u8 buffer[2];
	int ret;
...
static ssize_t __attribute__((unused)) touchkey_firm_store(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret, i;
...
//			gpio_tlmm_config(GPIO_CFG(GPIO_TSP_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	...
	ts->cmd_is_running = false;
	ts->cmd_state = 0;
	ts->config_fw_version = "I8268_Me_1112";
	g_config_fw_version = (u8 *)ts->config_fw_version;
